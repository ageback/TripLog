...
namespace TripLog.ViewModels
{
    public class BaseValidationViewModel : BaseViewModel, INotifyDataErrorInfo
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(propertyName)) return _errors.SelectMany(x => x.Value);
            if (_errors.ContainsKey(propertyName) && _errors[propertyName].Any()) return _errors[propertyName];
        ...
        protected void Validate(Func<bool> rule, string error, [CallerMemberName] string propertyName = "")
        {
            ...
            if (rule() == false) _errors.Add(propertyName, new List<string> { error });
